    ...
    {{-- @foreach( $colloques as $cle=>$col)
          ...
          <td class="center">
            <a href="{{ route('colloque.edit',$col->id)}} " class="btn btn-sm btn-success"><i class="ace-icon fa fa-pencil-square-o"></i></a>
            @if($col->getEtatID($col->etat) == null)
              <a href="/runcolloque/{{ $col->id }}" class="btn btn-sm btn-green" title="Déroulement"><i class="ace-icon fa fa-cog"></i></a>
              <a href="{{ route('colloque.destroy',$col->id) }}" data-method="DELETE" data-confirm="Etes Vous Sur ?" class="btn btn-sm btn-danger"><i class="ace-icon fa fa-trash-o></i>
            @endif
            </td>
          </tr>
          @endforeach --}}
