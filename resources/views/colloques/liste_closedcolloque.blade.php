...
<div class="col-xs-12 widget-container-col" id="widget-container-col-1"><br/>
     <div class="col-xs-12 widget-container-col" id="widget-container-col-12">
    	<div class="widget-box widget-color-blue" id="widget-box-12">
	    	...
	    	<tr>
	 	      <td><?= date('d M Y',strtotime($col["dat"].' monday next week')-1);?></td>
			    <td>{{$col["dat"]}}</td>
					<td>
					@foreach($col["membres"] as $i=>$m)
						<p class="text-primary">{{ $col["membres"][$i] }}</p></p> 
					@endforeach
					</td>
					<td>
						...
						<table class="collapse out budgets {{$j}}collapsed table-hover">
							<tr>
								<td style =" border: none!important;">
									...
									<div class="profile-info-value">
										<span>{{ $demande->demandeHosp->consultation->Date_Consultation }}</span>
									</div>
									</div>
								</td>
								<td class="">
									<div class="profile-info-row profile-user-info-striped">
									<div class="profile-info-name"><strong>Etat:</strong></div><div class="profile-info-value"><span>{{ $demande->demandeHosp->etat }}</span></div></div>
								</td>
							</tr>
							<tr>
							<td>
									...
									<div class="profile-info-value"><span>{{ $demande->medecin->nom }} {{ $demande->medecin->prenom }} </span>
									</div>
									</div>
								</td>
								<td class="">
									<div class="profile-info-row profile-user-info-striped">
										...
										<div class="profile-info-value"><span>{{ $demande->demandeHosp->modeAdmission }}</span></div>
									</div>
								</td>
							</tr>
								<tr>
							<td>
									<div class="profile-info-row profile-user-info-striped">
										...
										<div class="profile-info-value"><span>{{ $demande->demandeHosp->Service->nom }}</span></div>					
									</div>
								</td>
								<td class="">
									<div class="profile-info-row profile-user-info-striped">
										<div class="profile-info-name"><strong>Specialité:</strong></div>
										<div class="profile-info-value"><span>{{ $demande->demandeHosp->Specialite->nom }}</span>
									</div>
									</div>
								</td>
							</tr>
