...
<div class="page-header"><h1>Demandes de produits</h1></div>
<div class="row">
...
($demande->etat)!!}</td>
                    <td class="center">
					           ...
					           <button class="btn btn-danger btn-xs{!! $isInprog($demande)!!} deletedemande" title="Supprimer demande" data-id ="{{ $demande->id }}" data-confirm="Etes Vous Sur ?"><i class="fa fa-trash-o"></i></button>
										@endif
										@if(Auth::user()->is(10))
 ...
 <script type="text/javascript">
  ...
  function getAction(data, type, dataToSet) {
    ...
    if(data.etat == "En cours")
    {
      if('{{ Auth::user()->is(14) }}')
      {
        ...
        actions += ' <button class="btn btn-danger btn-xs deletedemande" title="Supprimer demande" data-id ="{{ $demande->id }}" data-confirm="Etes Vous Sur ?">  <i class="fa fa-trash-o"></i></button>';  
      }else
  ...
  $(function(){
    ...
    $('body').on('click', '.deletedemande', function (e) {
      e.preventDefault();
      var url = '{{ route("demandeproduit.destroy", ":slug") }}'; 
