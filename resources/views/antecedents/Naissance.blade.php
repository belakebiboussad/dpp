...
<div class="row">
	<div class= "widget-box widget-color-pink">
		<div class="widget-header">
		 <h5 class="widget-title bigger lighter"><font color="black"><i class="ace-icon fa fa-table"></i>Etats à la Naissance</font></h5>
			<div class="widget-toolbar widget-toolbar-light no-border">
        {{-- @if(count($exmonth) == 3) hidden @endif --}}
				<a id ="condAdd" class="btn-xs align-middle" data-toggle="modal" data-target="#condModal">
			...
			<div class="widget-main no-padding">
				<table class="table nowrap dataTable table-bordered no-footer table-condensed table-scrollable" id="bcondTab">
					<thead class="thin-border-bottom">
					  <tr>
					  	<th>Mois</th> <th>Score APGAR</th>
					    <th>Nombre de Cris</th><th>Poids</th>
					  	<th>Taille</th><th>Périmètre crânien</th>
					  	<th>Placenta</th><th class = "center"><em class="fa fa-cog"></em></th>
					  </tr>
			  	</thead>
			  	<tbody>
			  	  @foreach($patient->birthConditions as $key=>$cond)
			  		<tr id="{{ 'bcond'.$cond->id }}">
			  			<td>{{ $cond->month }}</td><td>{{ $cond->apgar }}</td>
			  			<td>
			  				@if($loop->first)
			  					{{ $cond->shoutnbr }}
			  				@else
			  			...
			  			<td>{{ $cond->bbWeight }}</td><td>{{ $cond->bbHeight }}</td>
			  			<td>{{ $cond->pcran }}</td><td>{{ $cond->placenta }}</td>
			  			<td class = "center">
