	...
	{{-- <div class="col-xs-12 widget-container-col" id="widget-container-col-1"><br/> --}}
        ...
        <tbody id ="rendez-VousBody" class="bodyClass">
				  ...
				  <tr>
						...
						<td>
								{{ $rdv->admission->demandeHospitalisation->consultation->Date_Consultation }}
						</td>
						<td>
								{{ $rdv->admission->demandeHospitalisation->DemeandeColloque[0]->medecin->Nom_Employe }}
								{{ $rdv->admission->demandeHospitalisation->DemeandeColloque[0]->medecin->Prenom_Employe }}
					 </td>
						<td class="center">
						  @switch( $rdv->admission->demandeHospitalisation->DemeandeColloque[0]->ordre_priorite)
   						    	@case(1)
        						...
        						@default
        										<span class="label label-sm label-success">
        											@break
	    		    @endswitch
			        {{ $rdv->admission->demandeHospitalisation->DemeandeColloque[0]->ordre_priorite }}</span>
					  <td>
					  	{{ $rdv->admission->demandeHospitalisation->DemeandeColloque[0]->observation }}
					  </td>
						<td><strong>{{ $rdv->date_RDVh }} {{ $rdv->heure_RDVh }} {{ $rdv->etat_RDVh  }}</strong></td>
